...
//						var_dump($min_max_block[$oneStock["block_id"]],$min,$max);die;
						/*end giá min max*/
...
function default_default(){
	...
	if($stock_type==_BLOCK_TYPE_HIGHLEVEL_SALE){
		...
		if($project_id > 0){
			$arrBlock = $clsProperty->getAll("`property_type`='_BLOCK' and for_id='{$project_id}' order by order_no ASC", $field);
			if(!empty($arrBlock)){
