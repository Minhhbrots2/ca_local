		...
		foreach($tblData as $key => $val){
			...
			if(!empty($email) && $clsClassTable->countItem("user_name='{$email}'") == 0){
				...
				$clsClassTable->insert(array(
					...
					'code' => $code,
					'user_name' => $email,
					'user_pass' => $clsClassTable->encrypt('skyrealty@2026'),
					'email' => $email,
...
function default_do_import(){
	...
	if($uid !== '' && file_exists($cachedFile)){
		...
		@unlink($cachedFile); // xoá ngay khi đã đọc xong: script chết giữa chừng cũng không bỏ lại rác trong cache/json
	}
