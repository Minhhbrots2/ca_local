...
//			_form.closest('.modal').remove();
	...
	}, delete_folder_price_sheets: function(_this, e){

		e.preventDefault();

		$Core.alert.confirm("Xác nhận xóa", "Bạn có chắc chắn muốn xóa Folder PTG này?", function(){

			toggleIndicatior(1);

			var $row = $(_this).closest(".group_price_sheets");

			if($(".group_price_sheets").length > 1){

				$row.remove();

				// giữ nút +Thêm ở dòng cuối nếu vừa xoá đúng dòng chứa nó

				if($(".group_price_sheets .add_folder_ptg").length === 0){

					$(".group_price_sheets:last label.col-form-label").first().append(' <a href="javascript:void(0);" class="add_folder_ptg" onClick="$Core.property.add_folder_price_sheets(this,event)">+Thêm</a>');

				}

			} else {

				// dòng duy nhất -> chỉ xoá dữ liệu, giữ dòng + nút +Thêm
...
$Core.setting = {
	...
	loadStaffConfirm : (_this,e) => {
		e.preventDefault();
		var confirm_name = $(_this).val(),
			toId = $(_this).attr("toId");
		if(confirm_name == "admin") {
			$(`#`+toId).removeClass("d-none");
		}else{
			$(`#` + toId).addClass("d-none").val("0");
		}
	}
