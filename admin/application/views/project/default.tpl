							...
							<div class="hastable">
								<table cellspacing="0" cellpadding="0" class="table table-striped table-vertical table_project" style="width:100%">
									<thead><tr>
										...
										<th class="text-left" width="200px">{$core->get_Lang('update')}</th>
										...
										<tr class="{if $smarty.section.i.index%2 eq 0}row1{else}row2{/if}">
											...
											<td class="text-center">
												<label class="switch" title="Hiển thị dự án ra menu">
													<input type="checkbox" tp="_project" for_id="{$project_id}" to_field="is_menu"{if $allItem[i].is_menu eq '1'} checked{/if} onclick="set_quick_menu(this, event)">
													<span class="slider round"></span>
												</label>
											</td>
											<td class="text-left text-nowrap">
												{$core->makeIcon('clock-o',$allItem[i].reg_date|date_format:"%d/%m/%Y %H:%M")}
											</td>
											<td class="text-center">
								...
								<div class="statistical d-flex align-items-center justify-content-between flex-wrap mt-2">
									<div class="d-flex align-items-center">
										...
										<select name="per_page" class="form-control w-auto" onchange="window.location = this.value">
											{foreach from=$allowPerPage item=pp}
											<option value="{$PCMS_URL}/{$link_page_perpage}&per_page={$pp}"{if $pp eq $recordPerPage} selected="selected"{/if}>{$pp}</option>
											{/foreach}
										</select>
										<span class="ml-3 text-muted">{$core->get_Lang('statistical')} <strong>{$totalRecord}</strong> {$core->get_Lang('records')}/<strong>{$totalPage}</strong> {$core->get_Lang('page')}. {$core->get_Lang('youareonpagenumber')} <strong>{$currentPage}</strong></span>
									</div>
									{if $totalPage > 1}
