							...
							<div class="col-md-5 text-right">
								<div class="d-flex justify-content-end gap-4 align-items-center">
									<div class="form-group w-200px mb-0">
										<select name="status_contract" id="" class="form-control form-select" onchange="$Core.project.saveStatusContract(this,event)" tp="Project" for_id="{$project_id}">
											<option value="">Chọn loại hợp đồng</option>
											{foreach from=$list_status_contract item=_oItem key=key name=i}
												<option value="{$_oItem.property_id}" {if $_oItem.property_id eq $more_information.status_contract} selected{/if}>{$_oItem.property_code}</option>
											{/foreach}
										</select>
									</div>
