...
//		console.log(arr_floor);
		...
		`, function(){	
			...
			if(floor_specical_new != ""){
				...
				for(let i=0; i<arr_floor_specical.length; i++){
					if(arr_floor.includes(arr_floor_specical[i])){
						$Core.alert.error("Tầng số "+arr_floor_specical[i]+" đã tồn tại");
						return false;
					}					
				}
				if(type == "_ADD") {
	...
	saveStatusContract : (_this,e) => {
		e.preventDefault();
		var tp = $(_this).attr("tp"), 
			for_id = $(_this).attr("for_id"), 
			_parent = $(_this).closest('.box_status_contract'),
			$_adata = {}, 
			status_contract = $(_this).val();
		$_adata["tp"] = tp;
		$_adata["for_id"] = for_id;
		$_adata["status_contract"] = status_contract;
		toggleIndicatior(1);
		$.post(path_ajax_script+'/index.php?mod=ajax&act=saveStatusContract', $_adata, function(respJson){
			toggleIndicatior(0);
		}, 'json');
		return false;
	},
