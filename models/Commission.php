...
class Commission extends DbBasic{
	...
	function crawl($spreadsheetId, $quarter_id, $sheet_name = '', $layout = ''){
		...
		// Hàng rào CỨNG: dừng trước cả nhánh dọn dòng cuối hàm. Nhánh đó chạy deleteByCond() xoá THẬT
		// mọi dòng hoa hồng của cả kỳ không khớp sheet, không lọc theo nguồn — số CRM tính ra sẽ bay sạch.
		// CHẶN MẶC ĐỊNH, không phụ thuộc config.php: thiếu hằng số thì vẫn chặn. Muốn lấy lại sheet làm
		// nguồn thì phải khai báo rõ _CRM_IS_COMMISSION_SOURCE = 0, chứ không phải nhờ hằng số biến mất.
		if(!defined('_CRM_IS_COMMISSION_SOURCE') || _CRM_IS_COMMISSION_SOURCE == 1){
			return "_disabled_crm_is_source";
		}
