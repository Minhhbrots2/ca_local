<?php
	...
	$lstAgency = $clsProperty->getAll($cond,$clsProperty->pkey.",title,more_information");
...
//	 $clsISO->print_pre($arr_crawl);die;
	if(!empty($arr_crawl)) {
		foreach ($arr_crawl as $key => $crawl) {
			...
			$arr_data = [
				"oneAgency"	=>	$crawl["oneAgency"],
				"config_column_data"	=>	$crawl["config_column_data"]
			];
//			echo "<pre>";
//			var_dump($spreadsheetId,$ranges,$target_id,$agency_id,_BLOCK_TYPE_LOWFLOOR_SALE,$crawl_lowfloor);die;
			$res = $clsCrawlLocal->getDataLowFloor($spreadsheetId,$ranges,$target_id,$agency_id,_BLOCK_TYPE_LOWFLOOR_SALE,$crawl_lowfloor);
