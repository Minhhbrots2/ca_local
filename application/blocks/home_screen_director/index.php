<?php 
	...
	// Cay phong ban Kinh doanh day du (toi tung phong KD) cho bo loc check-in
	$arr_dept_all = $clsProperty->getArraySearchByKey('_DEPARTMENT');
	$lstCheckinDept = array();
	$walkDept = function($parent, $depth) use (&$walkDept, $arr_dept_all, &$lstCheckinDept){
		if($depth > 8){ return; }
		foreach($arr_dept_all as $id => $row){
			if((int) $row['parent_id'] !== (int) $parent){ continue; }
			$lstCheckinDept[] = array('id' => (int) $id, 'title' => isset($row['title']) ? $row['title'] : '', 'depth' => $depth);
			$walkDept($id, $depth + 1);
		}
	};
	$walkDept(_DEPARTMENT_SALE_ID, 0);
	$smarty->assign('lstCheckinDept', $lstCheckinDept);
	##
