										...
										<div class="card-body">
											<div class="d-flex align-items-center justify-content-between">
												<div class="awe__project-info">
													<h4 class="card-title mb-2">
														<span class="text-dark text-fs-22 fw-semibold">{$_oProject.title}</span>
													</h4>
											...
											{if !empty($_oProject.list_blocks)}
												<div class="d-flex flex-wrap gap-1 text-fs-13 align-items-center text-muted">
													...
													{foreach from=$_oProject.list_blocks item=_oBlock key=k_block}
														{assign var=list_menu_buildings value=$_oBlock.list_menu_buildings}
														{assign var=more_block value=$_oBlock.more_information}
														{if !empty($list_menu_buildings)}	
															{if $start eq 0}
															<span class="text-muted"><i class='bx bx-table'></i> Tòa mở bán: </span>
															{/if}
															{foreach from=$list_menu_buildings item=_oBuild key=k_build name=i}
																{if $smarty.foreach.i.index gt 0}, {/if} <a href="{$_oBuild.link}" class="text-link text-nowrap" target="_blank">{$_oBuild.title}</a>
															{/foreach}
