		...
		<div class="modal-body">
			...
			<div class="form-row mb-2">
				...
				<div class="col-6 col-md-4">
					...
					<select class="form-control form-select required" name="sold_to_type">
						...
						{$clsProperty->getSelectByProperty('SALE_TYPE',$oneBilling.sold_to_type)}
					</select>
				</div>
			</div>
			...
			<div class="form-row mb-2">
				<div class="col-12 col-md-3 mb-2 mb-lg-0">
					...
					<select class="form-control form-select" name="deal_type" title="Nội bộ ăn TPKD/GĐKD, PTĐT ăn CV/TP/GĐ PTĐT">
						<option value="sale"{if $more_information.deal_type neq 'ptdt'} selected{/if}>Sale nội bộ</option>
						<option value="ptdt"{if $more_information.deal_type eq 'ptdt'} selected{/if}>Phát triển đối tác (PTĐT)</option>
					</select>
				</div>
				<div class="col-12 col-md-3 mb-2 mb-lg-0">
					<label class="form-label mb-1">Sales bán</label>
					<select class="required w-100{if $action eq '_add'} iso-selectizeImageSearch{else} iso-selectize{/if}" placeholder="Chọn sales bán" name="staff_id" data-optgroup="false"
						onChange="$Core.billing.handle_staff_changed(this, event)" data-url="{$PCMS_URL}/index.php?mod=home&act=list_staff&holderG=all">
						{if $action eq '_edit'}
						<option value="{$oneBilling.staff_id}" selected>{$clsProfile->getIndentity($oneBilling.staff_id)}</option>
						{/if}
					</select>
				</div>
				<div class="col-12 col-md-3 mb-2 mb-lg-0">
			...
			<div class="form-row mb-2">
				<div class="col-12">
					...
					{* Chia quá 100% thì hiện phần vượt thay vì kẹp về 0 — nếu không người nhập không biết sai. *}
