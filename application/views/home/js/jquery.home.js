...
function cancel_billing(_this, e){
	...
	$Core.messager.confirm('Xác nhận', 'Bạn chắc chắn muốn hủy giao dịch này?', function(){
		...
		$.post(PCMS_URL+'/index.php?mod='+MOD+'&act=cancel_billing', $_adata, function(html){
			...
			if(html.indexOf('_success') >= 0){
				window.location.reload();
			} else if(html.indexOf('_permission') >= 0){
				$Core.swal.error("Oops","Bạn không có quyền huỷ giao dịch này. Vui lòng liên hệ quản trị.");
			} else {
...
$Core.billing = $.extend($Core.global.billing, {
	init: () => {
		...
		/* Slot ẩn nằm xen giữa nên số in sẵn trong .tpl sẽ nhảy cóc sau khi xoá. */
		$('#co_sale_list .co-sale-row:not(.d-none) .co-index').each(function(i){
			$(this).text(i + 1);
		});
	}, update_co_sale_hint: () => {
		var sum = 0;
		...
		var main = 100 - sum,
			over = main < 0;
		/* Phần vượt trước đây bị ép về 0 nên người nhập không biết đã chia quá tay. */
		$('.co-main-chip').toggleClass('is-over', over);
		$('.co-main-label').text(over ? 'Vượt' : 'Sale chính giữ');
		$('.co_main_ratio').text(Math.round(Math.abs(main) * 100) / 100);
	}, save_info: (_this, e) => {
		e.preventDefault();
		var _validated = 0,
			_form = $(_this).closest('form'),
