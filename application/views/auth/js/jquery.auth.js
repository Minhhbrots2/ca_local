...
$(function(){
	...
	$('#frm-signin').validate({
		...
		submitHandler: function(form) {
			$.ajax({
				url: path_ajax_script+'/index.php?mod=' + MOD + '&act=checkExistAccount', // Đường dẫn API đăng nhập của bạn
				type: 'POST',
				dataType: 'json',
				data: $(form).serialize(),
				success: function(response) {
					console.log(response.msg);
					$(".error_user").html("").addClass("d-none");
					if (response.result) {
						window.location.href= response.link;
					} else {	
						var validator = $('#frm-signin').validate();
						if(response.type == "user_email") {
							validator.showErrors({
								user_email: response.msg
							});
						}else if(response.type == "user_pass") {
							validator.showErrors({
								user_pass: response.msg
							});
						}else{		
							$(".error_user").removeClass("d-none").html(response.msg);
						}
					}
				},
				error: function() {
