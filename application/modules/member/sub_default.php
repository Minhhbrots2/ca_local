...
function default_staff(){
	...
	$status_id = (int) Input::get('status_id', _STATUS_STAFF_ON_ID);
	...
	if(isset($_POST['filter']) && $_POST['filter'] =='filter'){
		...
		$status_id = (int) Input::post('status_id', _STATUS_STAFF_ON_ID);
...
function default_view_profile(){
	...
	if(!$clsProfile->canViewProfile($clsISO, $oneProfile, $profile_id, $dbProfile, $_profile_id)){
		echo json_encode(array(
			'uid' => $uid,
			'html' => '<div class="p-5 text-center text-muted"><i class="bx bx-lock-alt fs-1 d-block mb-2"></i>Bạn không có quyền xem hồ sơ nhân sự này.</div>'
		)); die();
	}
