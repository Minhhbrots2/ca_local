...
function helper_search_stock(){
	...
	if(($tp == '_search' && !empty($ms_code)) || ($tp=='_open' && $stock_id > 0)){
		...
		if(!empty($oneStock)){
			...
			$more_information = $clsISO->to_array_json($more_information);
			#- Cờ "Sale check qua Admin": badge cảnh báo trên card khi req_admin_check=1
			$req_admin_check = (int) $core->get_field($more_information, 'req_admin_check', 0);
			$html_req_admin = ($req_admin_check === 1)
				? '<div class="bg-warning text-dark text-center py-1 px-2 fs-12 fw-bold"><i class="bx bx-error-circle"></i> Căn cần liên hệ Admin để check trước khi tư vấn/chốt</div>'
				: '';
					...
					<div class="modal-body">
						...
						<div class="rounded-3 overflow-hidden">
							...
							<div class="py-2 px-3 bg-grayter">
								...
								<div class="d-flex flex-wrap gap-1 fs-12 mt-2 justify-content-center">
									'.($permiss_hide_stock_globe ? '<button stock_id="'.$stock_id.'" tp="poster" class="btn btn-sm btn-outline-'.($clsStock->checkShow($oneStock['show_website'], 'MOC')?'primary':'default').'" onClick="$Core.helper.hide_stock_MOC(this,event)" type="button">'.$core->makeIcon('check-square-o', 'MOC').'</button>' : '').($agency_id==_AGENCY_FH_ID && $permis_edit_stock_mechanism ? '<button type="button" p_field="is_sp_mech" stock_id="'.$stock_id.'" onclick="$Core.helper.set_stock_field(this,event)" class="btn btn-sm btn-'.((int)$core->get_field($more_information,'is_sp_mech',0)==1?'primary':'secondary').'">'.$core->makeIcon('bell-o', 'Bán nội bộ').'</button>':'').((!$clsISO->checkSale()) ? $html_request_ptg : "").'
									'.$html_button_more.'
								</div>':'').'
							</div>'.$html_zalo.'
...
function helper_update_quick_stock(){
	...
	if($tp=='sheet_price'){
		...
		if($clsStock->updateOne($stock_id, array(
			...
			$arr_user_request = $arr_request_ids = array();
...
function helper_load_top_ranking(){
	...
	if($clsCache->has($_cache_name) && 1==2){
		...
		$field = "SUM(`tbl_bl`.`commission_value`) as `total_price`,COUNT(DISTINCT `t1`.`billing_id`) as `total_billing`,`t2`.`profile_id`,`t2`.`more_information`";
		//$dbconn->debug=true;
		...
		//$clsISO->print_pre($list_top_ranking);die;
...
function helper_load_top_ranking25(){
	...
	$field = "SUM(`t1`.`realized_sales_compete`) as `total_price`,count(`t1`.`staff_id`) as `total_billing`,`t2`.`profile_id`";
	if($clsCache->has($_cache_name)){
		$lstItem = $clsCache->get($_cache_name);
		// $clsCache->delete($_cache_name);
	} else {
		$sql_query = "`t2`.`is_trash`=0 AND `t1`.`is_cancel`=0 AND `t1`.`billing_type`<>'"._BILLING_TYPE_SOP_ID."' 
		AND `t2`.`status_id`='"._STATUS_STAFF_ON_ID."' AND `t2`.`profile_id` NOT IN (".implode(',',_PROFILE_NOTIN_ID).") 
		AND (`t1`.`deposit_date` BETWEEN '{$start_date}' AND '{$due_date}') AND (`t2`.`list_department_id` NOT LIKE '%|"._DEPARTMENT_DEVELOP_ID."|%')";
		$lstItem = $dbconn->getAll("SELECT {$field} FROM {$clsBilling->tbl} as `t1` 
			INNER JOIN {$clsProfile->tbl} AS `t2` ON `t1`.`staff_id`=`t2`.`profile_id` AND `t2`.`department_id`<>'"._DEPARTMENT_DIRECTOR_ID."' 
			WHERE {$sql_query} GROUP BY `t2`.`profile_id` HAVING `total_billing`>0 ORDER BY `total_price` DESC LIMIT 0,25");
		$clsCache->put($_cache_name, (!empty($lstItem)?$lstItem:""), 15*60);
	}
	...
	if(!empty($lstItem)){
		...
		foreach($lstItem as $key => $val){
			...
			$oneStaff['department_name'] = $department_name;
			$lstItem[$key]['oneStaff'] = $oneStaff;
			$lstItem[$key]['total_price'] = $clsISO->shortNumber($val['total_price']);
			if($key == 0) {
				...
				$col_height= ($val['total_price'] * $ratio) / $max_height;
