...
function default_checkExistAccount(){
	global $assign_list,$_CONFIG,$core,$dbconn,$mod,$act,$_LANG_ID,$title_page,$description_page,$keyword_page,$clsConfiguration,$clsISO;
	$clsProfile = new Profile(); $assign_list["clsProfile"] = $clsProfile;
	#
	$result = true;
	...
	if(isset($_POST['submit']) && $_POST['submit'] =='signin'){
		...
		$type = "";
		if(empty($user_email)){
			$msg = 'Tên đăng nhập không để trống!';
			$type = "user_email";
			$result = false;
		}else if(!empty($user_email) && ($clsProfile->getIdByEmail($user_email) ==0)){
			$msg .= 'Tên đăng nhập không tồn tại!';
			$type = "user_email";
			$result = false;
		}else if(empty($user_pass)) {
			$msg = 'Mật khẩu không để trống!';
			$type = "user_pass";
			$result = false;
		}else {
			$tmp = $clsProfile->getByCond("`user_name`='{$user_email}' OR `email`='{$user_email}' and `user_pass`='{$clsProfile->encrypt($user_pass)}' limit 0,1", $clsProfile->pkey.",is_active,status_id");
			if(!empty($tmp)) {
				if($tmp["is_active"] != 1 || $tmp["status_id"] == _STATUS_STAFF_OFF_ID ) {
					$msg = 'Tài khoản không tồn tại hoặc bị khóa. <br>Liên hệ với bộ phận nhân sự để được hỗ trợ!';	
					$result = false;
				}else{
					$clsProfile->userLoggedIn($user_email,$user_pass);
				}
			}else{
