...
\*======================================================================*/
/**
 * Ai được động vào bảng hoa hồng (crawl sheet, đổi cấu hình nguồn, sửa/tạo dòng hoa hồng).
 * Các handler này trước đây không gate: bất kỳ tài khoản đăng nhập nào cũng POST thẳng được,
 * riêng commission_crawl còn nhận spreadsheetId tuỳ ý từ POST.
 */
function _commission_can_manage(){
	global $clsISO;
	if($clsISO->checkPermissionGroup('DIRECTOR')){
		return true;
	}
	return $clsISO->checkPermission('cash_book_access') ? true : false;
}
...
function commission_crawl(){
	...
	if(!_commission_can_manage()){
		echo json_encode(array('msg' => '_permission')); die();
	}
	$clsBilling = new Billing();
