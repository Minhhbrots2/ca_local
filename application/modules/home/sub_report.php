...
function report_load_reports_login(){
	...
	foreach ($lstProfile as $key => $val) {
		...
		$total_login = $clsProfileLog->countItem("`profile_id`='{$profile_id}' AND (`reg_date` BETWEEN {$start_time} AND {$end_time})");
		$total_search = $clsLog->countItem("`user_id`='{$profile_id}' AND (`reg_date` BETWEEN {$start_time} AND {$end_time})");
		$link_log = sprintf('%s?user_id=%s&start_date=%s&end_date=%s', 
			$clsISO->getLink("log-sale"), $profile_id, date("d/m/Y",$start_time), date("d/m/Y",$end_time)
		);
		$report_arrs[] = [
	...
	foreach($report_arrs as $key => $val){
		$html.= '<tr>
			...
			<td class="text-center fw-bold '.(($val["total_search"] == 0) ? 'text-main' : "text-success").'">
				'.$val["total_search"].' <a class="btn btn-outline-none p-1" href="'.$val["linkLog"].'" target="_blank"><i class="bx bx-link-external text-primary"></i></a>
			</td>
		</tr>';
	}
	// Return
