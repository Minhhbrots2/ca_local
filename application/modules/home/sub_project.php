...
function project_load_stock_popover(){
	...
	$html = '<div class="rounded-3 mb-2 overflow-hidden">
		...
		<div class="py-2 px-3 bg-grayter">
			...
			<div class="d-flex flex-wrap gap-1 fs-12 mt-2 justify-content-center">
				'.($permiss_hide_stock_globe ? '<button data-toggle="ripple" stock_id="'.$stock_id.'" class="btn btn-sm '.($clsStock->checkShow($oneStock['show_website'], 'MOC')?'btn-outline-primary':'btn-outline-default').'" onClick="$Core.helper.hide_stock_MOC(this,event)" type="button">'.$core->makeIcon('check-square-o', 'MOC').'</button>' : '').((!$clsISO->checkSale()) ? $html_request_ptg : "").'
				'.($agency_id==_AGENCY_FH_ID && $permis_edit_stock_mechanism ?'<button data-toggle="ripple" type="button" p_field="is_sp_mech" stock_id="'.$stock_id.'" onclick="$Core.helper.set_stock_field(this,event)" class="btn btn-sm '.((int)$core->get_field($more_information,'is_sp_mech',0)==1?'btn-primary':'btn-secondary').'">Cơ chế ĐB</button>':'').'
				'.$html_more_buttons.'
...
//				================
				if(empty($tmp)) {
					...
					if(empty($tmp)) {
						if($cat_id == _PROJECT_DOCS_POLICY_CATID || $cat_id == _PROJECT_DOCS_TRAINING_CATID) {
							$tmp = $clsProjectMeta->getAll("`is_trash`='0' AND `project_id`='{$project_id}' and (`cat_id`='{$cat_id}' or cat_id='"._PROJECT_DOCS_BM_CATID."') order by `reg_date` DESC");
						}else{
							$tmp = $clsProjectMeta->getAll("`is_trash`='0' AND `type`='project' AND `project_id`='{$project_id}' and (`cat_id`='{$cat_id}' or cat_id='"._PROJECT_DOCS_BM_CATID."') order by `reg_date` DESC");
						}
...
//				================
				if(empty($tmp)) {
					...
					if(empty($tmp)) {
						if($cat_id == _PROJECT_DOCS_POLICY_CATID || $cat_id == _PROJECT_DOCS_TRAINING_CATID) {
							$tmp = $clsProjectMeta->getAll("`is_trash`='0' AND `project_id`='{$project_id}' and `cat_id`='{$cat_id}' order by `reg_date` DESC");
						}else{
							$tmp = $clsProjectMeta->getAll("`is_trash`='0' AND `type`='project' AND `project_id`='{$project_id}' and `cat_id`='{$cat_id}' order by `reg_date` DESC");
						}
					}
				}
//				================
