...
function default_dashboard(){
	...
	if($tp=='total'){
		...
		$list_billings = $clsBilling->getAll("`is_trash`=0 and `is_cancel`=0 order by `reg_date` DESC limit 0,6");
		...
		if($skin == 'dbx'){
			if(!empty($list_billings)){
				...
				foreach($list_billings as $key => $val){
					...
					$project_name = $project_id > 0 ? $arr_project_cached[$project_id] : '';
					...
					$avatar = $clsProfile->getAvatar($staff_id, $arr_profile_cached[$staff_id]);
					$html.= '<div class="dbx-person">
						<div class="dbx-person__ava" data-url="/index.php?mod=home&act=load_profile_popover&user_id='.$staff_id.'" data-toggle="webui-popover" data-trigger="hover" data-width="300">
...
function default_open_billing(){
	...
	if($billing_id > 0){
		...
		if($core->get_field($more_information, 'deal_type', '') === ''){
			$more_information['deal_type'] = _billing_is_ptdt($_shares_form, $more_information) ? 'ptdt' : 'sale';
		}
...
function default_cancel_billing(){
	...
	if($oneBilling['is_cancel']==1){
		...
		if($clsBilling->updateOne($billing_id, array(
			...
			/** Cancel commission */
			$clsBilling->sync_commission($billing_id, array(), "cancel");		
			#activity log			
...
function default_pop_save_billing(){
	...
	if($billing_id > 0){
		...
		AND `project_id`='{$project_id}' AND `billing_id`<>'{$billing_id}'")){
			...
			$more_information['is_sendemail'] = (int) Input::post('is_sendemail', 0);
			$more_information['is_sendpale'] = (int) Input::post('is_sendpale', 0);
			$more_information['commission_value'] = $clsISO->processSmartNumber($commission_value);
			$more_information['bonus_agency'] = $clsISO->processSmartNumber($bonus_agency);
			$more_information['hot_bonus'] = $clsISO->processSmartNumber($hot_bonus);
...
function default_pop_save_news(){
	...
	if($news_id > 0){
		...
		if($is_post_moc == 1){
			$news_moc_id = (int) $core->get_field($more_information, "news_moc_id", 0);
			if($news_moc_id > 0){
				$clsNews->updateOne($news_moc_id, array_merge($more, array(
					'cat_id' => $cat_moc_id,
					'title' => $title,
				...
				$news_moc_id = $clsNews->getMaxId();
				$clsNews->insert(array_merge($more, array(
					$clsNews->pkey => $news_moc_id,
					'user_id' => $profile_id,
					'user_id_update' => $profile_id,
					'cat_id' => $cat_moc_id,
					'domain_id' => _CAT_MOC_ID,
					'title' => $title,
					'slug' => $core->replaceSpace($title),
					'order_no' => $order_no,
					'content' => $content,
				...
				$more_information['news_moc_id'] = $news_moc_id;
			}
		}
		...
		if($clsNews->insert(array_merge($more, array(
			...
			#gui email toi toan bo cong ty
			if(!empty($send_email)) {
				$send = $clsNews->sendEmail($news_id);
			}					
			#activity log			
			$clsActivityLog = new ActivityLog();
				...
				<div class="d-flex w-100">
					...
					<div class="awe__comment-item-body">
						...
						<div class="awe__comment-item-action d-flex align-items-center mt-1">
							<a href="javascript:void(0);" class="awe__comment-like-button control-action" table_id="'.$table_id.'" clsTable="'.$clsTable.'" comment_id="'.$comment_id.'"><span class="awe__comment-total-liked">0</span></a>
	...
	if($skin == 'dbx'){
		...
		if(!empty($list_top_staff)){
			...
			foreach($list_top_staff as $key => $val){
				...
				$avatar = $clsProfile->getAvatar($profile_id, $val, 40, 40);
...
function default_open_billing_settlement(){
	...
	$is_ptdt = _billing_is_ptdt($shares, $more_information);
	$rows = array();
	...
	foreach($shares as $_oShare){
		...
		if(!$is_ptdt && ($head_of_dep_id <= 0 || $sale_dir_id <= 0)){
		...
		$rows[] = array(
			...
			'ctv_rate' => ((float) $core->get_field($_oShare, 'ctv_rate', 0)) > 0 ? (float) $_oShare['ctv_rate'] : '',
	...
	$smarty->assign('st_agency_rate', $core->get_field($more_information, 'agency_rate', ''));
	$smarty->assign('st_agency_amount', $core->get_field($more_information, 'agency_amount', ''));
	...
	$smarty->assign('st_is_ptdt', $is_ptdt ? 1 : 0);
	$smarty->assign('st_ptdt_cv_id', (int) $core->get_field($more_information, 'ptdt_cv_id', 0));
	$smarty->assign('st_ptdt_tp_id', (int) $core->get_field($more_information, 'ptdt_tp_id', 0));
	$smarty->assign('st_ptdt_gd_id', (int) $core->get_field($more_information, 'ptdt_gd_id', 0));
	$oneProject = $clsProject->getOne((int) $oneBilling['project_id'], "`title`");
	// Đưa R đã giải sẵn (có fallback sang giá trị bán) xuống form để JS khỏi lặp lại logic fallback
...
function default_save_billing_settlement(){
	...
	// "Sales chịu" là ô TỰ TÍNH CHO SỬA ĐÈ: để trống nghĩa là trả về tự tính, không phải ép bằng 0.
	// Không tách ca này thì processSmartNumber('') = 0 sẽ thành một giá trị đè hợp lệ.
	if(array_key_exists('total_deduction_sales', $_POST)){
		if(trim((string) Input::post('total_deduction_sales')) === ''){
			unset($more_information['total_deduction_sales']);
		} else {
			$more_information['total_deduction_sales'] = $clsISO->processSmartNumber(Input::post('total_deduction_sales'));
		}
	}
	...
	// PTĐT: cấp giao dịch, khoá riêng — không đụng dep_logs (đang là khoá lọc quyền dashboard)
	_billing_set_if_posted($more_information, 'ptdt_cv_id', (int) Input::post('ptdt_cv_id', 0));
	_billing_set_if_posted($more_information, 'ptdt_tp_id', (int) Input::post('ptdt_tp_id', 0));
	_billing_set_if_posted($more_information, 'ptdt_gd_id', (int) Input::post('ptdt_gd_id', 0));
	if(array_key_exists('ptdt_cv_id', $_POST)){
		$more_information['ptdt_manual'] = 1; // Admin đã chọn tay → handler lưu GD không được suy lại đè lên
	}
	...
	$realized_primary = 0;
	$total_sales_amount = $total_sales_net = 0;
	foreach($shares as $_oShare){
		...
		// Số tiền tự tính, nhưng gõ đè được vì có ca CTV nhận tiền khoán không theo %.
		$ctv_rate = isset($posted_shares[$sid]['ctv_rate']) ? $clsISO->convertToNumber($posted_shares[$sid]['ctv_rate']) : 0;
		$ctv_amount_raw = isset($posted_shares[$sid]['ctv_amount']) ? trim((string) $posted_shares[$sid]['ctv_amount']) : '';
		$ctv_amount = ($ctv_amount_raw === '')
			? round($calc['realized'] * $ctv_rate / 100)
			: $clsISO->processSmartNumber($ctv_amount_raw);
	...
	// Đại lý tính trên R của cả căn, tự tính nhưng cho gõ đè
	$agency_amount_raw = trim((string) Input::post('agency_amount'));
	if(array_key_exists('agency_amount', $_POST)){
		$more_information['agency_amount'] = ($agency_amount_raw === '')
			? round($r_base_save * $clsISO->convertToNumber($core->get_field($more_information, 'agency_rate', 0)) / 100)
			: $clsISO->processSmartNumber($agency_amount_raw);
	}
	$msg = '_error';
	if($clsBilling->updateOne($billing_id, array(
		'more_information' => json_encode($more_information, JSON_UNESCAPED_UNICODE),
