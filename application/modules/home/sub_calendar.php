...
function calendar_load_calendar_today(){
	...
	$more_information = $oneProfile['more_information'];
	$field = "{$clsBilling->pkey},`stock_code`,`contract_date`,`contract_status_id`,`estimate_date`
	,`agree_date`,`billing_source_id`,`billing_type`,`agree_status_id`,`more_information`,`admin_id`,`staff_id`,`project_id`";
	...
	$is_edit = $total = $total_signed = $total_unsigned = 0;
	...
	if(!empty($list_billings)){
		...
		foreach($list_billings as $key => $val) {
			...
			$list_billings[$key]["project_name"] = $arr_cache_project[$project_id];
			$sign_ts = ($contract_date > 0) ? $contract_date : $agree_date;
			$list_billings[$key]["time_hm"] = ($sign_ts > 0) ? date('H:i', $sign_ts) : '';
			if(strpos($status, 'Đã ký') !== false){ ++$total_signed; }
			else if(strpos($status, 'Chưa ký') !== false){ ++$total_unsigned; }
			++$total;
		}
	}
//	$clsISO->print_pre($list_billings);die;
	...
	echo json_encode(array("html" => $html,"total" => $total,"total_signed" => $total_signed,"total_unsigned" => $total_unsigned));die;
}
