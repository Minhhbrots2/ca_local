	...
	if($disp == 'chart'){
		...
		if(!empty($list_departments)){
			foreach($list_departments as $key => $val){
				...
				$field = "SUM(`tbl_bl`.`commission_value`) AS `total_sales`,COUNT(DISTINCT `t1`.`billing_id`) AS `total_billings`";
				...
				AND `t2`.`status_id`<>'"._STATUS_STAFF_OFF_ID."' AND ({$cond_dep_chart})".$sql_time);
		...
		$sql_other = "select sum(`tbl_bl`.`commission_value`) as `total_sales`,count(distinct `t1`.`billing_id`) as `total_billings`
				from `{$clsBillingSale->tbl}` as `tbl_bl`
		...
		$sql_partner = "select sum(`tbl_bl`.`commission_value`) as `total_sales`,count(distinct `t1`.`billing_id`) as `total_billings`
				from `{$clsBillingSale->tbl}` as `tbl_bl`
				inner join {$clsBilling->tbl} as `t1` on `t1`.`billing_id`=`tbl_bl`.`billing_id`
			...
			foreach($list_departments as $key => $val){
				...
				$tmp = $dbconn->getRow("SELECT {$field}
					...
					INNER JOIN {$clsProfile->tbl} AS `t2`
						ON `t2`.`profile_id`=`tbl_bl`.`staff_id`
					WHERE `t1`.`is_trash`=0 AND `t1`.`is_cancel`=0 AND `t2`.`is_trash`=0
					AND (`t2`.`department_id`='{$department_id}'".(!empty($arr_deps)? " OR `t2`.`department_id` IN (".implode(",", $arr_deps).")" : "" ).")
					AND `t2`.`status_id`<>'"._STATUS_STAFF_OFF_ID."'".$sql_time);
			...
			foreach($arr_departments as $okey => $oval){
				...
				$html.= '<div class="d-flex align-items-center justify-content-between py-1/5 px-2 bg-white-100 rounded-2 mb-1">
					...
					<div class="d-flex gap-2 text-white text-center align-items-center">
						...
						<div class="text-center w-px-80">'.$clsISO->shortNumber($oval['total_sales']).'</div>
					</div>
				</div>';
				++$ii;
			}
