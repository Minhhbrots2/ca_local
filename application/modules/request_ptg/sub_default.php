...
function default_requestPTG(){
	...
	,$clsButtonNav,$clsConfiguration,$clsISO,$profile_id,$oneProfile;
	$clsRequestPTG = new RequestPTG();
	$clsProfile = new Profile();
	$clsStock = new Stock();
	...
	if($action == "_OPEN") {
		...
		$list_admins = $clsProfile->getAll("`is_trash`=0 and `is_active`='1' and `department_id`='"._DEPARTMENT_MARKETING_ID."' 
			and `status_id`<>'"._STATUS_STAFF_OFF_ID."' and {$clsProfile->pkey}<>'{$profile_id}'", $field);
...
function default_exist_ptg(){
	...
	if(!empty($list_requests)){
		...
		if(!empty($arr_user_requestMOC)) {	
			$titleNoty = sprintf('<strong>Quản trị viên</strong> báo căn [<strong>%s</strong>] đã có phiếu tính giá',  $oStock['ms_code']);
			...
			foreach ($arr_user_requestMOC as $member_id) {
				$clsNotifyMOC->sendEmailRequestPTG($member_id,$oStock['ms_code'],"exist");
			}			
		}
	}
	// Return
