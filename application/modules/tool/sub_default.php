			...
			foreach($list_items as $key => $val){
				...
				if($user_id == $profile_id){
					...
					if(!isset($arr_profile_cached[$user_id])){
						$field = "`first_name`,`last_name`,`full_name`,`avatar`";
				...
				<div class="js__calendar-tool js__calendar-action">
					<div class="js__calendar-cmd w-px-25 d-flex gap-1 align-items-center">
						...
						<a data-toggle="ripple" class="cursor-pointer" onClick="$Core.calendar.delete(this, event)" calendar_id="'.$val[$clsCalendar->pkey].'" calendar_id="'.$val[$clsCalendar->pkey].'" title="Xóa"><i class="bx bx-x"></i></a>
					</div>
				</div>':'';
...
function default_save_calendar(){
	...
	#
	if($start_date < $current_now || $start_date >= $end_date){
		$msg = "_invalid";
	} else {
		if($calendar_id > 0){
			if($is_fullday == 1){
				$tmp = $clsCalendar->getByCond("`is_trash`=0 AND {$clsCalendar->pkey}<>'{$calendar_id}' 
					AND FROM_UNIXTIME(`start_date`,'%Y-%m-%d')='{$regis_date}'");
			} else {
				$tmp = $clsCalendar->getAll("`is_trash`=0 AND {$clsCalendar->pkey}<>'{$calendar_id}' 
					AND FROM_UNIXTIME(`start_date`,'%Y-%m-%d')='{$regis_date}' 
			...
			if($is_fullday == 1){
				$tmp = $clsCalendar->getByCond("`is_trash`=0 AND FROM_UNIXTIME(`start_date`,'%Y-%m-%d')='{$regis_date}'");
			} else {
				$tmp = $clsCalendar->getAll("`is_trash`=0 AND FROM_UNIXTIME(`start_date`,'%Y-%m-%d')='{$regis_date}' 
				AND (`start_date`<'{$end_date}') AND (`end_date`>{$start_date})");
			}
			if(!empty($tmp)){
				$msg = '_duplicated';
...
function default_delete_calendar(){
	...
	$msg = "_error";
	$calendar_id = (int) Input::post('calendar_id', 0);
	if($clsCalendar->deleteOne($calendar_id)){
		$msg = "_success";
	}
	// Return
	echo $msg; die();
